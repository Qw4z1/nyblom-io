...
When setting out to write something, there is one hurdle that seems extremely difficult for some to pass. That is to actually start writing.
...
The problem with that, apart from the fact that I wasn’t actually producing anything of value to anyone but me, is that no matter how much information I got I had no idea if other people would actually like the idea that I had.
...
But you see, writing is editing. You are almost guaranteed to not get it right the first time. So when starting out, the goal shouldn’t be to get it right. The goal should be to get started.

This version is for your eyes only. This means that you also have the option of throwing it away and write a shitty second draft. Heck, most of the times when I write a blog post I end up splitting my shitty first draft into three separate blog posts, of which one gets discarded the very next day.
...
Once in a blue moon though, you luck out and end up with something that has some semblance of a real piece of writing. Then you have the option to start tweaking it. To gather feedback, and really let it take form.
...
If you know me, you probably know that I like mental modelsThis post is not at all about writing. Blog posts is a useful framing for the technique, but I apply it in other areas of my life where I have trouble getting started.
...
Or to quote [Seth Godin](https://seths.blog/):
