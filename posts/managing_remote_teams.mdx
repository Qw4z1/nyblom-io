...
Instead people can do their work asynchronously and report back when it's done. This has one very big implication for how we work. Namely that most of the time, exactly when the work gets done *does not really matter*. This shift in mindset means we can go deep into the work, with less context shifts.
